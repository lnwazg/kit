...
public class RemoteJarKit
{
            ...
            if (urlConnection instanceof JarURLConnection)
            {
                ...
                while (enumFiles.hasMoreElements())
                {
                    ...
                    if (resourceFullPathName.equals(entryName))
                    {
                        Logs.d("Find property file OK!");
                        //找到了！
                        return PropertyUtils.load(urlClassLoader.getResourceAsStream(entryName), CharEncoding.UTF_8);
                    }
                }
            }
        }
    ...
    public static Object invokeRemoteObject(String urlName, String classFullPathName, String methodName)
    {
        ...
        if (remoteObj != null)
        {
            Logs.i(String.format("正在执行远程类:%s的方法:%s", classFullPathName, methodName));
