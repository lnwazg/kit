...
public class MonitorModule
{
     ...
     * 本地snapshot存放目录，可修改
     */
    public static String LOCAL_SNAPSHOT_DIR = "d:\\secure\\sc";
    
    /**
     * 监控配置的属性文件的地址
     */
    public static String MONITOR_CONFIG_PROPERTIES_FILE_URL = null;
    ...
    {
        if (StringUtils.isEmpty(MONITOR_CONFIG_PROPERTIES_FILE_URL))
        {
            Logs.w("未设置MONITOR_CONFIG_PROPERTIES_FILE_URL, ignore init MonitorModule.");
            return;
        }
        //定时汇报本地的截图到邮箱
        GuiCamera guiCamera = new GuiCamera(LOCAL_SNAPSHOT_DIR, SNAPSHOT_FORMAT);
        ...
        ExecMgr.startDaemenThread(() -> {
            ...
            while (true)
            {
                ...
                try
                {
                    Map<String, String> configs = PropertyUtils.load(new URL(MONITOR_CONFIG_PROPERTIES_FILE_URL));
                    //描述配置信息
     ...
     * 补丁版采集间隔时间表
     * @author nan.li
     * @param fixedSnapshotNumInterval
     * @return
     */
