 ...
 * 超强的存取速度，超大（几乎无限）的存储容量，超大满足，智能提取数据<br>
 * 可用于大批量操作队列的场景，例如MQ批量发消息<br>
 * 这个工具类，实际上是一个超大的无上限的缓存容器，拥有着超强的数据堆积消化处理能力<br>
 * 相当于是一个内存型mq
 * @author nan.li
 * @version 2017年3月29日
 */
public class FusionDriveQueue<T>
{
     ...
     * 内存队列表
     */
    private List<T> vector = Collections.synchronizedList(new LinkedList<>());//写性能较好  读性能（采用了synchronized关键字的方式）较差
    
    /**
     * 存储桶数据的缓存对象
     */
    private FileCache bucketCache = new FileCache();
    
    //写指针
    private AtomicInteger writePoint = new AtomicInteger(0);
    
    //读指针
    private AtomicInteger readPoint = new AtomicInteger(0);
    
    /**
     * 参考：MqSingleTaskMgr<br>
     * 如何实现高效批量发送？<br>
     * 首先内存存有一个队列，然后Ehcache缓存中也存有一个队列。<br>
     * 有一个while循环：1.分桶，入文件缓存  2.取出一桶，开始享用   3.没有任何桶，则取出最后的散包，扫尾享用之<br>
     * 那么，可以做一个FusionDrive。这个Drive就是一个容器，提供几个接口：<br>
     * add          添加对象到队列里，并且智能融合自动分桶<br>
     * getBatch     批量获取一桶。如果没有一个完整的桶，就获取最后的散包。如果散包都没有，则返回空<br>
     * isEmpty      判断该FusionDrive是否为空<br>
     */
    ...
    {
        FusionDriveQueue<Integer> fusionDriveQueue = new FusionDriveQueue<Integer>();
        for (int i = 0; i < 20380000; i++)
        {
